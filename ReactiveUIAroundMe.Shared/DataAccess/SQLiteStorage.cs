// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SQLiteStorage.cs" company="Flush Arcade Pty Ltd.">
//   Copyright (c) 2015 Flush Arcade Pty Ltd. All rights reserved.
// </copyright>
// --------------------------------------------------------------------------------------------------------------------

namespace ReactiveUIAroundMe.Shared.DataAccess
{
	using System;
	...
	public class SQLiteStorage : ISQLiteStorage
	{
		...
		/// The sqlite platform.
		/// </summary>
		private readonly object _sqlitePlatform;
		...
		/// <summary>
		/// Gets or sets a value indicating whether this <see cref="T:ReactiveUIAroundMe.Portable.DataAccess.SQLiteStorage"/> is initialised.
		/// </summary>
		...
		public async Task Setup()
		{
			if (!Initialised)
			{
				...
				// create DB tables
				await CreateTable<ApplicationStateStorable>();
				await CreateTable<IdentityStorable>();
			...
			var connectionFactory = new Func<SQLiteConnectionWithLock>(() =>
				{
					if (_conn == null)
					{
						_conn = new SQLiteConnectionWithLock(_sqlitePlatform as ISQLitePlatform, 
		...
		public async Task InsertObject<T>(T item) where T : class, IStorable, new()
		{
			using (var releaser = await asyncLock.LockAsync())
			{
				try
				{
					var insertOrReplaceQuery = item.BuildInsertOrReplaceQuery();
